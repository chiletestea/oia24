...
export default function LandingPage() {
  ...
  const [activeModal, setActiveModal] = useState<'exercises' | 'programs' | 'brujula' | null>(null);
  ...
  // Tu Brújula — herramientas de orientación emocional (reflexión, no diagnóstico)
  const herramientas = [
    {
      id: 'inventario-gottman',
      title: 'Inventario Gottman',
      icon: '💞',
      desc: 'Evalúa los 4 jinetes del conflicto en tu relación de pareja',
      path: '/chat?exercise=gottman',
    },
  ];
    ...
    <div className="min-h-dvh w-full overflow-x-hidden bg-gradient-to-br from-purple-900 via-teal-900 to-teal-800">
      ...
      <div className="min-h-dvh flex flex-col justify-center items-center px-4 py-12 pt-[max(3rem,env(safe-area-inset-top))] pb-[max(3rem,env(safe-area-inset-bottom))] relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl w-full mx-auto text-center">
          ...
          {/* CUATRO BURBUJAS - columna en móvil, fila con wrap en pantallas >= sm para evitar solapamientos */}
          <div className="flex w-full max-w-md flex-col items-center gap-5 sm:max-w-none sm:flex-row sm:flex-wrap sm:items-center sm:justify-center sm:gap-4 mb-8">
            {/* Ejercicios */}
            <button
            ...
            {/* Tu Brújula */}
            <button
              onClick={() => setActiveModal('brujula')}
              className="w-36 h-36 md:w-40 md:h-40 shrink-0 rounded-full bg-gradient-to-br from-violet-400/30 to-violet-500/10 border-2 border-white/20 flex flex-col items-center justify-center cursor-pointer hover:scale-110 hover:border-violet-500/60 hover:shadow-2xl transition-all duration-300 text-white font-semibold text-sm text-center p-2 leading-tight"
            >
              <span className="text-3xl mb-1">🧭</span>
              <span>Tu<br />Brújula</span>
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[85dvh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b p-4 sm:p-6">
              <div className="flex justify-between items-center">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900">🧭 Tu Brújula</h2>
                <button
                  onClick={() => setActiveModal(null)}
                  aria-label="Cerrar"
                  className="-m-2 p-2 text-gray-500 hover:text-gray-700 text-2xl leading-none"
              ...
              <p className="mt-1 text-xs sm:text-sm text-gray-500">
                Herramientas de orientación emocional — reflexión, no diagnóstico
              </p>
            </div>
            <div className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              {herramientas.map((herramienta) => (
                <button
                  key={herramienta.id}
                  onClick={() => {
                    router.push(herramienta.path);
                    setActiveModal(null);
                  }}
                  className="border-2 border-violet-500 rounded-lg p-4 text-center cursor-pointer hover:bg-violet-50 hover:shadow-lg transition-all duration-300"
                >
                  <div className="text-4xl mb-2">{herramienta.icon}</div>
                  <h3 className="font-bold text-gray-900 text-sm mb-1">{herramienta.title}</h3>
                  <p className="text-xs text-gray-600">{herramienta.desc}</p>
                </button>
              ))}
            </div>
          </div>
